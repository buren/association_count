...
module AssociationCount
  def association_count(counted_model, distinct)
    proc do |base_model, counted_model, distinct|
      ...
      distinct_string  = distinct ? 'DISTINCT' : ''
      joins(counted_table.to_sym).
      select("#{table_name}.*, COUNT(#{distinct_string} #{counted_table}.id) as #{counted_name}_count_raw").
      group("#{table_name}.id")
  ...
  def can_count(model_name, opts = {})
    ...
    distinct = true unless opts[:distinct] === false
    class_eval do
      scope "include_#{singular_name}_count", -> { association_count(reflection.klass, distinct) }
    end
  end
end
